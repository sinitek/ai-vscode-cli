...
export const WEBVIEW_I18N = {
  en: {
    ...
    helpModesGeneralItem:
      "Use the lightest mode that can safely handle the task: Vibe for direct work, Loop when the whole batch should finish before one combined review, Loop+ when each finished subtask is accepted on its own, and Graph for visible DAG orchestration.",
    ...
    helpModeVibeCons: "Cons: no explicit subtask orchestration, weaker progress visibility, and less protection for large multi-step work.",
    ...
    helpModeLoopCons: "Cons: slower and more token-heavy than Vibe; dependencies and conflicts are less explicit than Graph.",
    ...
    helpModeLoopPlusPros: "Pros: one execution end starts acceptance immediately; other completions that arrive during acceptance enter a visible queue and are accepted one by one. After acceptance, more tasks can be added, or the run waits while other tasks are still running.",
    helpModeLoopPlusCons: "Cons: an execution end is not acceptance complete, and there is no single combined review.",
    ...
    helpModeGraphCons: "Cons: highest setup cost and UI complexity; still no graph editor.",
  ...
  "zh-CN": {
    ...
    helpModesGeneralItem:
      "优先使用能安全完成任务的最轻模式：Vibe 做直接对话与小改，Loop 等整批执行结束后集中复核，Loop+ 在单个子任务执行结束后逐个验收，Graph 做可视 DAG 编排。",
    helpModeVibeTitle: "Vibe",
    helpModeVibeBest: "适合：快速问答、小范围修改、读代码、探索性排查。",
    helpModeVibePros: "优点：启动最快、开销最低、单个对话里最容易即时调整方向。",
    helpModeVibeCons: "缺点：没有显式子任务编排，进度可见性较弱，不适合很大的多阶段任务。",
    helpModeLoopTitle: "Loop",
    ...
    helpModeLoopCons: "缺点：比 Vibe 更慢、更耗 token；依赖关系和冲突边界不如 Graph 明确。",
    helpModeLoopPlusTitle: "Loop+",
    helpModeLoopPlusBest: "适合：希望每个子任务一执行结束就单独验收，而不按统一轮次等待整批。",
    helpModeLoopPlusPros: "优点：一个子任务执行结束就立即开始验收；验收期间其它完成进入可见队列，再逐个验收。验收后可以追加任务；没有新任务但仍有任务在运行时继续等待。",
    helpModeLoopPlusCons: "缺点：执行结束不等于验收完成，也没有整批统一复核。",
    helpModeGraphTitle: "Graph",
    helpModeGraphBest: "适合：复杂任务，需要明确节点、依赖、并行批次和证据可视化时使用。",
    helpModeGraphPros: "优点：状态最清楚，任务边界最强，调度更确定，失败后的恢复控制更完整。",
    helpModeGraphCons: "缺点：准备成本和界面复杂度最高；目前还没有图编辑器。",
