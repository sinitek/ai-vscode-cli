...
export type LoopDebatePromptTarget = {
  cli: string;
};

export const LOOP_DEBATE_MIN_PARTICIPANTS = 2;
export const LOOP_DEBATE_MAX_PARTICIPANTS = 6;

export const LOOP_DEBATE_SUGGESTED_PARTICIPANTS: ReadonlyArray<LoopDebateParticipantDefinition> = [
  {
    id: "blue_planner",
    role: LOOP_DEBATE_BLUE_TEAM_ROLE,
    title: "蓝队方案方",
    focus: "提出可执行方案，明确目标、约束、成功标准，并主动回应红队质疑。",
  },
  {
    id: "red_attacker",
    role: LOOP_DEBATE_RED_TEAM_ROLE,
    title: "红队攻击方",
...
function getLoopDecisionSubtasks(decision: LoopMainDecision): import("./loopTaskStore").LoopSubtaskDecision[] {
...
export function buildLoopDebateBriefMarkdown(
  task: LoopTaskRecord,
  target: LoopDebatePromptTarget,
  round: number,
  ...
  const lines: string[] = [
    ...
    `- 上一轮 estimatedRemainingRounds：${formatLoopEstimatedRemainingRounds(task.estimatedRemainingRounds) ?? "未记录"}`,
    `- brief 文件：${paths.briefFile}`,
    `- 群聊记录文件：${paths.chatFile}`,
    `- 最大安全发言批次数：${LOOP_DEBATE_MAX_DIALOGUE_TURNS}`,
    `- 裁判主持人角色：${LOOP_DEBATE_MODERATOR_TITLE}`,
    "",
    "## 原始目标",
    task.rootPrompt,
    "",
    ...
    `- 裁判主持人每次最多点名 ${LOOP_DEBATE_MAX_BATCH_SPEAKERS} 位参与者进入下一批发言；未被点名的角色本批次不得发言。`,
    ...
    `- 参与者数量范围：${LOOP_DEBATE_MIN_PARTICIPANTS}-${LOOP_DEBATE_MAX_PARTICIPANTS}`,
    "- 清单必须至少包含 1 个蓝队和 1 个红队；后续群聊只按裁判主持人选定的参与者推进。",
    "",
    "## 可参考的红蓝原型",
    ...LOOP_DEBATE_SUGGESTED_PARTICIPANTS.map((participant) => (
...
export function buildLoopDebateInitialChatMarkdown(
  task: LoopTaskRecord,
  target: LoopDebatePromptTarget,
  round: number,
  paths: LoopDebatePaths,
  ...
  const lines: string[] = [
    ...
    `- 最大安全发言批次数：${LOOP_DEBATE_MAX_DIALOGUE_TURNS}`,
    `- 裁判主持人：${formatLoopGroupChatMemberName(LOOP_DEBATE_MODERATOR_TITLE)}`,
    `- 裁判主持人 ID：${LOOP_DEBATE_MODERATOR_ID}`,
    "",
    "## 群聊规则",
    "- 每位角色必须读取本文件中已有发言后再输出自己的下一条发言。",
    `- 群聊称呼必须和成员列表名称一致，发言、点名和互相回应时使用 ${formatLoopGroupChatMemberName("成员名称")} 格式；成员 ID 只用于 JSON、artifact 文件名和调度字段，不作为群聊称呼。`,
...
export function buildLoopDebateParticipantRosterChatMarkdown(
  ...
  const lines: string[] = [
    ...
    ...participants.flatMap((participant) => [
      `## 参与者加入：${formatLoopGroupChatMemberName(participant.title)}`,
      "",
...
export function buildLoopDebateChatTurnMarkdown(
  dialogueTurn: number,
  ...
  return [
    "",
    `## 发言：${formatLoopGroupChatMemberName(participantTitle)}`,
    "",
    `- 成员 ID：${participantId}`,
    `- 群聊发言批次：${dialogueTurn}/${LOOP_DEBATE_MAX_DIALOGUE_TURNS}`,
    "",
    artifactText.trim(),
    "",
  ].join("\n");
...
export function buildLoopDebateModeratorTurnMarkdown(
  dialogueTurn: number,
  artifactText: string,
): string {
  return [
    "",
    `## 主持人控场：${formatLoopGroupChatMemberName(LOOP_DEBATE_MODERATOR_TITLE)}`,
    "",
    `- 成员 ID：${LOOP_DEBATE_MODERATOR_ID}`,
    `- 群聊发言批次：${dialogueTurn}/${LOOP_DEBATE_MAX_DIALOGUE_TURNS}`,
    "",
    artifactText.trim(),
    "",
...
export function buildLoopDebateDialogueTurnChatEventMarkdown(
  round: number,
  dialogueTurn: number,
  maxDialogueTurns: number,
  previousDecision: LoopDebateModeratorDecisionRecord | null,
  currentSpeakers: readonly LoopDebateParticipantDefinition[],
): string {
  const speakerLine = currentSpeakers.length > 0
    ? `- 本批次点名发言者：${currentSpeakers.map((speaker) => formatLoopGroupChatMemberName(speaker.title)).join("、")}`
    : "- 本批次点名发言者：未指定";
...
export function buildLoopDebateRuntimeForcedFinalizeMarkdown(
  ...
  return [
    ...
    "## 运行时强制收束",
    "",
    `${formatLoopGroupChatMemberName(LOOP_DEBATE_MODERATOR_TITLE)}最终动作：${decision.action}`,
    `${formatLoopGroupChatMemberName(LOOP_DEBATE_MODERATOR_TITLE)}理由：${decision.reason}`,
    "",
  ].join("\n");
}

export function buildLoopDebateFinalParticipantMarkdown(
  participantId: string,
  participantTitle: string,
  artifactText: string,
): string {
  return [
    "",
    `## 最终立场：${formatLoopGroupChatMemberName(participantTitle)}`,
    "",
    `- 成员 ID：${participantId}`,
    "",
    artifactText.trim(),
...
export function buildLoopDebateDialogueClosedMarkdown(
...
function buildLoopDebateSubtaskSummaryLines(task: LoopTaskRecord): string[] {
  ...
  return [
    `- 已完成：${formatLoopDebateSubtaskList(completed)}`,
    `- 运行中：${formatLoopDebateSubtaskList(running)}`,
    `- 失败/阻塞：${formatLoopDebateSubtaskList(failed)}`,
    `- 待处理：${formatLoopDebateSubtaskList(pending)}`,
  ];
}

function formatLoopDebateSubtaskList(subtasks: LoopSubtaskRecord[]): string {
...
export function buildLoopDebateParticipantDisplayPrompt(
  round: number,
  dialogueTurn: number,
  title: string,
  finalPass: boolean,
  ...
  return finalPass
    ...
    : `Loop 辩论第 ${round} 轮群聊 ${dialogueTurn}/${LOOP_DEBATE_MAX_DIALOGUE_TURNS}：${title}`;
}

export function buildLoopDebateParticipantRosterModelPrompt(
  task: LoopTaskRecord,
  round: number,
  paths: LoopDebatePaths,
): string {
  const suggestedParticipants = LOOP_DEBATE_SUGGESTED_PARTICIPANTS
  ...
  return [
    ...
    `主沟通文件：${task.mainCommunicationFile}`,
    `子任务沟通目录：${getLoopCommunicationPaths(task.id).subtasksDir}`,
    "",
    "职责和限制：",
    ...
    `3. 必须设计 ${LOOP_DEBATE_MIN_PARTICIPANTS}-${LOOP_DEBATE_MAX_PARTICIPANTS} 个参与者。复杂任务可更多，简单任务可更少。`,
    ...
    `6. 新清单中的 role 只能取：${LOOP_DEBATE_BLUE_TEAM_ROLE} / ${LOOP_DEBATE_RED_TEAM_ROLE}。${LOOP_DEBATE_PARTICIPANT_ROLES.filter((role) => role !== LOOP_DEBATE_BLUE_TEAM_ROLE && role !== LOOP_DEBATE_RED_TEAM_ROLE).join(" / ")} 仅用于兼容旧任务记录，不要新写。`,
    ...
    `{"artifactFile":${JSON.stringify(paths.participantRosterFile)},"summary":"红蓝组队摘要","openingSpeakerIds":["blue_planner"],"participants":[{"id":"blue_planner","role":"${LOOP_DEBATE_BLUE_TEAM_ROLE}","title":"蓝队方案方","focus":"提出可执行方案并回应红队攻击"},{"id":"red_attacker","role":"${LOOP_DEBATE_RED_TEAM_ROLE}","title":"红队攻击方","focus":"寻找假设漏洞、证据缺口、边界条件和阻塞风险"}]}`,
    "",
    "原始目标：",
    task.rootPrompt,
  ].join("\n");
...
export function buildLoopDebateParticipantModelPrompt(
  task: LoopTaskRecord,
  round: number,
  dialogueTurn: number,
  maxDialogueTurns: number,
  ...
  const teamGuidance = participant.role === LOOP_DEBATE_BLUE_TEAM_ROLE
    ? [
  ...
  return [
    ...
    `群聊称呼：${formatLoopGroupChatMemberName(participant.title)}`,
    `关注重点：${participant.focus}`,
    `任务记录文件：${task.taskStoreFile}`,
    `brief 文件：${paths.briefFile}`,
    `群聊记录文件：${paths.chatFile}`,
    ...
    "4. 你不能直接输出最终 LoopMainDecision JSON；最终决策由共识汇总器在读取完整群聊后生成。",
    `5. 你必须点名回应 chat.md 中至少一个已经发言的其他角色；点名时使用 ${formatLoopGroupChatMemberName("成员名称")}，不要用英文成员 ID 作为称呼；蓝队优先回应红队攻击点，红队优先继续攻击蓝队尚未修正的方案。`,
    ...
    "",
    "## 点名回应",
    `列出你回应了哪些角色的哪些观点；角色称呼必须使用 ${formatLoopGroupChatMemberName("成员名称")}；若暂无其他角色发言，写“暂无，作为首位发言者开场”。`,
    "",
    "## 追问或修正",
...
export function buildLoopDebateModeratorDisplayPrompt(
  round: number,
  dialogueTurn: number,
  maxDialogueTurns: number,
): string {
...
export function buildLoopDebateModeratorModelPrompt(
  task: LoopTaskRecord,
  round: number,
  dialogueTurn: number,
  maxDialogueTurns: number,
  ...
  return [
    ...
    `9. 群聊称呼必须和成员列表名称一致；写点名追问、群聊态势和理由时使用 ${formatLoopGroupChatMemberName("成员名称")}，不要用英文成员 ID 作为称呼。`,
    "",
    "artifact 必须包含以下固定小节，标题必须完全一致：",
    "## 群聊态势",
    ...
    "",
    "## 点名追问",
    `如果 action=continue，列出下一批次需要蓝队或红队回答什么问题；成员称呼使用 ${formatLoopGroupChatMemberName("成员名称")}；如果不继续，写“无”。`,
    "",
    "## 下一批发言者",
    `如果 action=continue，列出 1-${LOOP_DEBATE_MAX_BATCH_SPEAKERS} 个下一批被点名发言的参与者 id，每行一个；如果不继续，写“无”。`,
    "",
    "## 主持人决策",
...
export function buildLoopDebateConsensusModelPrompt(
  task: LoopTaskRecord,
  round: number,
  paths: LoopDebatePaths,
