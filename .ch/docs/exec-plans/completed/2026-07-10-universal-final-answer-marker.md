...
- 单元自测结果：`npm run build` 通过；最小相关回归 46/46 通过；排除两个已知失败文件后的扩展回归 336/336 通过。
- 失败处理记录：首次扩展回归被重命名前遗留的 `dist/test/codexFinalAnswerPolicy.test.js` 重复执行，产生 2 个过期断言失败；删除该生成产物后通过。最终全量为 388 项、365 通过、23 失败，仍集中在未改动的 `configService.test.js` 1 项和 `lobsterBoundaryRecord.test.js` 22 项，归类为范围外历史失败。
...
已完成。默认严格策略在三种 CLI 普通任务上统一接受结构化 final 或 `[final_answer]` assistant 文本，普通任务首轮与 hidden retry prompt 都注入标记约定；Loop 等机器协议保持原有结构化终态，兼容策略和旧设置迁移保留。实现、国际化、事实来源、Node build、最小回归与扩展回归均已验证，全量测试的 23 个范围外历史失败已记录。
