...
Loop主任务还存在专门的最终总结气泡：主任务完成时扩展会移除最终 JSON 协议气泡，并追加 `loopFinalSummary=true` 的 assistant Markdown 最终总结气泡。该气泡缺失时，同样不能把任务视为真正完成。
...
- Loop任务只有在最终总结气泡已经写入并展示后才被视为完成；若任务记录显示完成但主任务对话缺少最终总结气泡，扩展自动续跑“继续”。
...
- [x] 已完成Loop任务若主任务消息中已有 `loopFinalSummary=true`，不会重复追加或重复续跑。
- [x] 已完成Loop任务若缺少最终总结气泡，扩展不会直接结束，会自动以“继续”恢复同一任务。
...
- 风险：缺少气泡时无限续跑。
- 缓解：普通任务复用统一 hidden retry 上限；Loop任务在成功写入 `loopFinalSummary=true` 后立即结束，异常则回到 needs-review/error 路径。
...
- 扩展验证：静态检查 `runLoopPrompt` 完成态、恢复态和最终总结消息写入链路。
- 回归验证：`node --test dist/test/hiddenRetry.test.js`。

## 测试与清单同步

- 单元测试：当前Loop编排大量依赖 VS Code 扩展运行态和本地任务存储，先做构建与 hidden retry 纯函数回归验证；若后续抽出纯函数再补Loop编排单测。
- 功能清单：已同步普通 hidden retry 与Loop模式能力规格。
...
- 2026-06-04：最终结论气泡以 `ChatMessage.loopFinalSummary=true` 作为权威判断，避免依赖文案或 UI class。
...
已完成普通任务最终结论气泡兜底和Loop `loopFinalSummary=true` 最终总结兜底。非主动结束且缺少气泡时会继续运行；手动停止不会自动继续；存在气泡时不会重复续跑。
