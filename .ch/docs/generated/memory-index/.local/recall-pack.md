...
- `.ch/docs/design-docs/loop-debate-multi-agent-mode.md` | Loop 红蓝辩论多智能体模式详细设计 | matches=loop, prompt
  Why: 与当前 focus 相关的设计决策入口，命中：loop / prompt。
...
- `.ch/docs/design-docs/loop-plus-scheduling.md` | Loop+ 完成事件调度 | matches=loop, prompt
  Why: 与当前 focus 相关的设计决策入口，命中：loop / prompt。
...
- `.ch/docs/design-docs/graph-orchestration-mode.md` | Graph 编排模式详细设计 | matches=loop, prompt
  Why: 与当前 focus 相关的设计决策入口，命中：loop / prompt。
