...
- AI 对话和 Loop 群聊不再展示自动睡眠气泡、倒计时、计划唤醒时间或提前唤醒入口；用户通过现有“继续执行”手动恢复任务。群聊继续确认框会让用户选择恢复任务记录的原 CLI 分组、配置档案、主/子模型和思考力度，或使用当前 Loop 模式配置的主/子模型；没有原配置快照的旧任务只能使用当前配置。Loop 与 Loop+ 的“我要说话”确认框不展示该选择。
