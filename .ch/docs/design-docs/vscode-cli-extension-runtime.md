...
- AI 对话面板的“新建会话”图标在指针停留 3 秒后向宿主查询当前仍存活的 Codex app-server 长连接数。计数来自 app-server 连接池里 `isAlive()` 为真的进程，不按会话或 thread 重复计算；指针离开后隐藏提示，过期查询结果不显示。短停留仍只使用原来的“新建会话”提示。
