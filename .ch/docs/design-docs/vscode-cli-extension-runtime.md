...
- `src/extensionHost/promptInteractiveRuntime.ts`：Codex / Claude interactive host，当前 1398 行。它负责 `CodexInteractiveRunner` / `ClaudeInteractiveRunner` 的 turn 内消息、runner 事件映射、session adoption/migration、停止收口、消息持久化、subagent progress、Codex 结构化人工交互请求拦截、Codex/Claude Vibe 自然语言人工交互兜底、hidden retry 和 final answer 回归路径。
...
- `codexRunner.ts`：通过 `codex app-server --listen stdio://` 建立 JSON-RPC 会话，stdout/stdin 只按 LF/CRLF 拆 NDJSON，并把 U+2028/U+2029 转义后写入，避免行分隔符截断 JSON；维护主 threadId；模型或配置档案切换时仍续接已映射 thread，并把当前 TOML 根级 `model_provider` 传给 `thread/start` / `thread/resume` 的 `modelProvider`，避免插件侧主动切断服务端上下文；`turn/start` 仅传模型覆盖。`item/agentMessage/delta`、`collabAgentToolCall`、`subAgentActivity` 和 `turn/completed` 均按通知 `threadId` 区分父线程与子线程，子线程输出进入独立子代理气泡，不得改写主 threadId、主任务列表、父回复 final 标记或父 turn 完成状态；只有主线程且 `turnId` 匹配当前主 turn 的 `turn.completed status="completed"` 会通过专用回调进入 `promptInteractiveRuntime`；该信号只表示回合结束，不能单独生成最终回复气泡。若该 completed turn 的最后一条主线程 `agent_message` 为 `phase:null` / 缺省 phase 且其后没有工具活动，runner 会把该条非 thinking assistant 正文提升为 `codexFinalAnswer=true`。普通任务最终气泡只认 `final_answer` 类型或非 thinking 正文包含 `[final_answer]`。Runner 对 JSON-RPC request 暴露 `onRequest` 拦截点，默认仍按 app-server fallback 处理 unsupported request；Codex Vibe/coding 且全局 `humanInteractionEnabled=true` 时，runner 启动 app-server 会追加 `--enable default_mode_request_user_input`，`initialize.capabilities.experimentalApi` 也会置为 `true`，随后 `promptInteractiveRuntime` 拦截 `item/tool/requestUserInput` 与 `mcpServer/elicitation/request` 并按 Codex `answers: { <questionId>: { answers: string[] } }` / MCP `action`-`content` 契约回传。Codex、Claude 和 OpenCode Vibe/coding 在用户原始 prompt 明确要求 AI 先询问需求、但模型只返回普通问题列表时，会分别由 `promptInteractiveRuntime` / `promptOneShotRuntime` / `promptParallelRuntime` 做自然语言兜底：移除该问题列表 assistant 气泡，复用同一 Webview 表单收集补充信息；兜底会把“可选 / 选项 / 例如 / 如”候选项和紧随问题的 `A.` / `B.` / `C.` 字母选项列表渲染为 radio/checkbox；提交后把补充信息作为下一轮输入继续当前 Codex thread、Claude session 或 OpenCode one-shot/parallel 会话，拒绝或关闭表单会终止当前任务且不进入 hidden retry；Loop/Graph 不走该弹窗。Runner 优先直接启动已解析的 Codex 可执行路径，显式注入 `CODEX_HOME` / `CODEX_HOME_DIR`，启动前确保工作区 trust。相同启动参数的会话共用一条 app-server 连接，只 `initialize` 一次；已加载 thread 的后续回合、hidden retry 和压缩只发 `turn/start` 或 `thread/compact/start`，回合结束不退出进程。已加载回合的停止发 `turn/interrupt`。冷启动失败、dispose 或进程退出才关闭连接；“常用命令 -> 压缩上下文”对 Codex 直接复用当前 threadId 发送 `thread/compact/start`，且全局工具设置“执行后自动压缩上下文”（默认开启）会在已有会话任务成功结束且执行超过 5 分钟后自动触发同一路径；任务中断、报错或执行不超过 5 分钟不触发
- AI 对话面板的“新建会话”图标在指针停留 3 秒后向宿主查询当前仍存活的长连接总数。总数等于 Codex app-server、Claude 流式会话和 OpenCode `serve` 三类进程里仍存活的连接数，不按会话或 thread 重复计算；指针离开后隐藏提示，过期查询结果不显示。短停留仍只使用原来的“新建会话”提示。
...
- 环境变量脱敏
- 按全局工具设置 `historyRetentionDays` 保留日志（默认 30 天，范围 1–3650 天）
- 10MB 分段切割

`src/extension.ts` 同时负责对工作区级历史数据执行保留策略：

- 未收藏历史提示词按全局 `historyRetentionDays` 保留；收藏提示词不参与自动清理
- 历史会话按全局 `historyRetentionDays` 保留，并清理对应消息文件与交互映射
- `tasks.json` 中的任务运行记录按全局 `historyRetentionDays` 保留
