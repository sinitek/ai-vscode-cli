...
- `src/extensionHost/promptInteractiveRuntime.ts`：Codex / Claude interactive host，当前 1398 行。它负责 `CodexInteractiveRunner` / `ClaudeInteractiveRunner` 的 turn 内消息、runner 事件映射、session adoption/migration、停止收口、消息持久化、subagent progress、Codex 结构化人工交互请求拦截、Codex/Claude Vibe 自然语言人工交互兜底、hidden retry 和 final answer / completed-turn 回归路径。
