...
- Codex 主线程思考气泡消费 app-server `item/reasoning/summaryTextDelta`、`item/reasoning/textDelta` 和 `item/reasoning/summaryPartAdded`，按 `itemId` 拼接后写入 thinking assistant 气泡；Grok 等模型若把 `[final_answer]` 草稿写进 reasoning summary，插件只保留标记前的思考正文，且 `item.completed` 不得用更短摘要覆盖已流式内容。子线程 reasoning 不进入父回复。
