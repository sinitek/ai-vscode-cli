...
- Codex 主线程思考气泡消费 app-server `item/reasoning/summaryTextDelta`、`item/reasoning/textDelta` 和 `item/reasoning/summaryPartAdded`，按 `itemId` 拼接后写入 thinking assistant 气泡。同一 reasoning item 的后续 delta 继续写入原气泡，中间插入工具 trace 或普通回复也不会把这段思考拆成半句新气泡。模型名包含 `grok` 时，`thread/start` 和 `thread/resume` 的 config 会设置 `model_reasoning_summary=detailed`：kedaya 对 `summary=auto` 常把 reasoning summary 截成约 200 字再加 `...`，`detailed` 才能带回完整摘要。Grok 等模型若把 `[final_answer]` 草稿写进 reasoning summary，插件只保留标记前的思考正文，且 `item.completed` 不得用更短摘要覆盖已流式内容。子线程 reasoning 不进入父回复。
