...
- 工具设置中的全局项（`debug`、`autoAddEditorContextTags`、`autoCompactContextAfterRun`、`multiAgentEnabled`、`locale`、`macTaskShell`、`loopMaxRounds`、`loopAutoCloseSubtaskTabs`）写入 `~/.sinitek_cli/settings.json`。
...
该能力只进入 `loop` / Loop 编排链路；普通 `coding` 模式不读取该目录。首版没有新增设置、按钮、状态气泡或其他 UI，也没有新增 i18n 文案。
