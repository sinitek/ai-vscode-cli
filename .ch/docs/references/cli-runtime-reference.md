...
- Codex 的主任务、子任务、裁判主持人、参与者、共识汇总、续跑与唤醒统一读取 `PromptRunInput.model`；旧主/子模型持久化字段只兼容读取，不进入 PanelState、消息 payload 或运行时。
