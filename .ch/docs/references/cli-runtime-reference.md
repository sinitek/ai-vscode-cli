...
- 工具设置中的全局项（`debug`、`autoAddEditorContextTags`、`autoCompactContextAfterRun`、`multiAgentEnabled`、`locale`、`macTaskShell`、`loopMaxRounds`、`loopSubtaskMaxThinkingMode`、`loopAutoCloseSubtaskTabs`）写入 `~/.sinitek_cli/settings.json`。
...
- Loop 群聊显式继续以主任务 Tab 当前 CLI 分组为权威：即使 Tab 已切换分组，也通过任务原 CLI 的消息/session 绑定定位同一 Tab，再读取目标 CLI 当前激活配置和模型。跨 CLI 恢复保留同一 `taskId`，并在运行前把任务的 CLI/session Store 归属迁移到目标分组；`needs-review`、`error`、`stopped` 中断终态优先结束主 Tab 运行态，旧编排轮次和重试在派发前必须检查终态，不能在主动停止后重新写成 `running`。
