...
- 面板“工具设置”不提供最终答复判定策略。普通任务 prompt 固定要求 `[final_answer]` 协议文本；旧 `finalAnswerPolicy`、`codexFinalAnswerPolicy` 与历史兼容值会被忽略，不能改变运行时终结判定。Codex 交互式普通任务除显式 `phase:"final_answer"` / `codexFinalAnswer=true` 和 assistant 文本标记外，还接受由 `codexRunner` 确认为主线程、当前主 turn 且状态为 `turn.completed status="completed"` 的结构化完成信号；现有 Grok-style 保守文本 fallback 继续保留。子线程 `turn.completed`、`failed`、`interrupted`、主动停止、Claude/OpenCode 不走该 Codex completed-turn fallback。
