...
- 适用对象：需要维护 Codex harness starter 或评估外部 Skills 来源的开发者。
- 目标：记录值得长期关注的外部 Skill 来源、适用边界和引入前检查项。
- 维护要求：外部候选不记录个人安装痕迹；本仓库的 Skills 目录不构成 Loop 子任务的运行时输入。
...
## 已移除的 Loop Workflow Skill 快照

`media/loop-workflow-skills/`、`src/loopSkillGuidance.ts` 以及对应同步/校验脚本已于 2026-07-15 删除。Loop 主任务继续读取项目规则；子任务不会从仓库 Skills 目录、用户 Home 或扩展内置资源加载 Skills，而是遵循主任务提供的自包含指令并在隔离执行根中运行。

`.agents/skills/`、`media/workspace-scaffold/.agents/skills/`、`media/official-skills/**` 与 `media/official_skills_catalog.json` 仍分别服务于仓库开发、用户主动安装的 scaffold 或配置中心 Skills 管理，但都不是 Loop 子任务输入。
...
外部 skill 候选只记录值得引入或评估的上游能力，不用于拆分仓库内置说明书。仓库内置 core skills 以 `.agents/skills/AGENTS.md` 为准。
