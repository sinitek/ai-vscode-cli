...
- Prompt 历史记录保存在插件本地历史存储；历史记录弹窗“历史提示词”页签支持展开、复用和星标收藏，收藏状态持久化到记录字段 `favorite`，旧记录缺失字段时按未收藏读取。页签顶部提供共用关键词搜索框和“仅收藏”过滤，关键词实时过滤提示词正文、CLI、记录 ID，以及历史会话的首条提示词、标签、CLI、会话 ID，匹配大小写不敏感；搜索状态仅保存在当前 Webview UI 状态，不修改历史数据或收藏状态。页签同时显示收藏计数，过滤后无结果展示独立中英文空态文案；“清空未收藏”只删除未收藏提示词，收藏提示词不被手动清空、30 天 retention 或数量上限裁剪删除；通过“加入列表”进入队列的提示词会在入队时立即写入历史提示词列表，队列后续自动或手动执行时不会重复写入
