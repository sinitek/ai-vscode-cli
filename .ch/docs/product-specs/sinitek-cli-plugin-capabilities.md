...
- Codex / Claude / OpenCode 的普通任务 prompt 与 hidden retry prompt 都会追加统一最终回复约定：任务真正完成后，最终回复必须以 `[final_answer]` 开头，过程更新和非最终回复不得使用该标记；该内部约定不会改变 AI 对话中展示的原始用户问题。Loop 主任务/子任务等已有独立机器协议与结构化完成气泡的内部运行不会注入或要求该文本标记，避免破坏纯 JSON 决策解析。普通任务的最终答复协议固定为结构化/协议收口：Codex 显式 `phase:"final_answer"` / `codexFinalAnswer=true`、OpenCode 同一 `messageID` 的非 thinking assistant `text` 与 `step_finish.reason="stop"`、当前用户消息之后包含 `[final_answer]` 的非 thinking assistant 文本，以及 Codex 交互式主 turn 的 `turn.completed status:"completed"` 都可收口。Codex completed-turn fallback 只由 runner 在确认主线程和当前主 turn 后传递，子线程 completed、子代理 assistant 气泡、`failed`、`interrupted`、主动停止、Claude/OpenCode 普通成功退出都不能借此收口；现有 Codex 保守文本 fallback 继续保留。OpenCode 的 `tool-calls` 阶段、跨 message ID 的正文与 `stop`、无正文 `stop` 和纯 thinking 文本都不能通过。thinking、trace、system、user、旧回合消息和失败/中断状态也不能收口。对非主动中断/异常，或 CLI 成功退出但本轮仍没有最终结论信号的情况，会沿既有规则隐式发送“继续/continue”自动重试最多 5 次，间隔依次为 5 秒、15 秒、30 秒、2 分钟、5 分钟；不会展示隐式用户消息；每次失败进入下一次自动重试前会追加错误 trace 气泡和排队提示，真正开始重试时再追加开始提示并恢复标签运行态；达到上限后展示最近一次真实错误
