...
- 开关默认关闭，只在工具设置的“工作区”页签配置并写入 workspace settings；解析配置时采用“显式 false 防误开优先”，兼容旧 `memoryEnabled`、`globalMemoryEnabled`、`workspaceMemoryEnabled` 字段。只有显式开启并确认初始化后，才会安装 scaffold 与触发 CodeGraph 设置。Harness 安装状态的 UI 判定保持简单：当前工作区存在 `.ch` 目录即视为已安装，开关展示为已勾选且禁用。CodeGraph 开关同样用复选框：点击后开始安装，CLI、MCP 交互和工作区 `.codegraph/` 索引就绪后自动勾选且禁用。
