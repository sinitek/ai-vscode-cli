...
- Loop+ 子任务启动后，主任务聊天页会看到与经典 Loop 相同的“沟通文件”消息，群聊 transcript 追加“子任务加入”。子任务结束时，群聊追加“子任务发言”：有最终回复时展示回复，并额外附带沟通文件路径；没有回复时沿用经典 Loop 的运行结果和沟通文件说明。主任务聊天页同时追加一条结束消息，中英文界面都带沟通文件路径。失败或停止的子任务同样留下这条消息。辩论任务不把这些事件写入红蓝 `chat.md`。
