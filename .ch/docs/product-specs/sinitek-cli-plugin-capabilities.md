...
- 发送区发送按钮左侧提供土黄色时钟入口，用于创建一次性 AI 对话定时任务；弹窗默认下一天 00:00，可填写提示词、选择当前 CLI/模式并添加附件。任务保存到 `~/.sinitek_cli/scheduled-tasks.json`，附件复制到 `~/.sinitek_cli/scheduled-attachments/<taskId>/`，扩展启动和工作区切换时恢复当前工作区任务，到点执行时按目标 CLI 当时的 `interactiveMode`（如 `loop` / `graph`）复用对应 Prompt 执行链，Loop 子模式同样按当时配置读取，并记录 `completed` 或 `failed`；`running` 任务在重启后恢复为 `pending`，终态记录按全局 `historyRetentionDays` 清理
