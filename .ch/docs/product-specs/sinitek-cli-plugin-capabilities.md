...
- 不提供远程服务端托管
- OpenCode 作为新增支持目标接入；具体交互 Runner、会话续接与上下文压缩能力以当前实现为准
