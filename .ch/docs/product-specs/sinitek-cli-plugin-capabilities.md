...
- Codex 任务执行时，底部运行条在“流式消息”按钮右侧以与“提示词”同高度的徽章显示“上下文：”前缀和当前已用上下文。数值来自 app-server `thread/tokenUsage/updated`（以及 `turn/completed` 快照）的 `last`，按官方 `tokens_in_context_window() = totalTokens - reasoningOutputTokens` 计算，单位为 `k`；开跑后先显示占位，有读数后保留到下次开跑前。子线程用量不计入。Claude / OpenCode 不展示该读数。
