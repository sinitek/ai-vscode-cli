...
- 日志保留与临时文件清理
- 插件管理的历史痕迹（logs / 未收藏 prompt history / session history / task runs / loop task records / loop communications）按工具设置“自动清理”页中的全局 `historyRetentionDays` 保留，默认 30 天、范围 1–3650 天；收藏 prompt history 保留到用户取消收藏后再按普通历史规则清理。临时目录仍按独立的 1 小时策略清理
- Webview 渲染失败时的回退页
