...
- Graph 模式已完成 Phase 1 最小运行内核与 Phase 2 恢复交互：Webview 模式下拉可选择 Graph，发送后端进入 `interactiveMode=graph` 的 `runGraphPrompt` 分支；Graph 入口默认不注入插件侧长期记忆 recall，节点 `runPrompt` 结束也不会自动写入长期记忆。扩展先创建 planning-only Graph run，仅包含保留 `plan` AI planner 节点；planner 必须在节点 communication file 的 `## JSON` 中输出 `plannedGraph.nodes` 和 `plannedGraph.edges`，宿主校验后 materialize 为 AI 规划的 realized DAG（可包含分支、fan-out/fan-in、测试、评审、human_gate、sleep、merge 和 summary），规划无效时 run 进入 `needs-review` 而不执行固定线形 fallback。planner 需要输出中文节点标题；宿主对常见英文标题做中文兜底，GraphRunPanel 打开旧持久化 run 时也按同一规则显示中文标题。Graph 使用 `~/.sinitek_cli/graph-runs/<workspaceKey>/<cli>/<session-or-__pending__>/<graphRunId>/graph-runs.json`、`~/.sinitek_cli/graph-communications/<graphRunId>/graph.json`、`events.jsonl` 和 `nodes/<nodeId>.md` 保存状态、事件和节点沟通文件。节点执行继续复用现有 CLI runner / `runPrompt` 路径，并以 `taskRole="subtask"` 优先在 Graph 专用独立 git worktree cwd 中运行；如果本机没有 git、当前工作区不是 git repo、git 不支持 worktree 或 worktree 创建失败，则自动降级为 `executionMode=direct` 并在当前工作区直接执行。每个节点 prompt 都注入全图拓扑、当前位置、节点/边清单、直接上下游、上游/下游链路、同批 active 节点、冲突线索和后续 test/review/merge/summary 职责边界；每个节点结束后宿主会读取节点 communication file 的 `## JSON` 作为真实状态。worktree 模式会创建本地 checkpoint commit，node record 保存 `executionCwd` / `worktreeCwd` / `baseCommit` / `commit` 用于回退和重试；direct 模式只保存 `executionCwd`，不创建 checkpoint commit。worktree 模式下整个 Graph run `completed` 时，宿主会把 worktree 最终 HEAD 通过 `git merge --squash` 合回当前工作区分支，保留为未提交改动供用户审阅/提交，并在合回成功后删除 Graph worktree；若 `graph-worktrees` 父目录已空也一并删除，同时 prune worktree 注册项并删除对应 `sinitek-graph-*` 分支；目标工作区已有不相关未提交内容时仍让 Git 尝试合回。direct 模式没有 merge-back/cleanup 步骤，完成态表示改动已直接写入当前工作区。若 worktree 模式下 worktree 缺失、Graph diff 会覆盖本地改动、发生冲突、合并失败或 cleanup 失败，run 会回到 `needs-review` 并在 events / 系统消息中记录原因。
...
- GraphRunPanel 只渲染真实接通且当前状态允许的操作：Continue / Resume sleeping、needs-review 或 error run，“我要说话”补充消息，Retry failed / blocked node，Feedback rollback failed / blocked 验证类节点，Approve human_gate，Stop run。补充消息会写入 Graph run 的 `supplementalRequirements`、主沟通文件和 events，并注入后续 Graph 节点 prompt；不承诺立即打断已经运行中的节点。Retry 在 worktree 模式下会回滚到目标节点执行前 `baseCommit` 并清理未跟踪文件，再重新调度节点；direct 模式下 Retry 只会在当前工作区状态上重跑，不做 git reset。Feedback rollback 会优先按 active `review_feedback` / `if_fail` 边或上游依赖选择可回退工作节点，并且只在 worktree/baseCommit 可用时回滚到上游节点 `baseCommit`，将上游节点及其下游重置为 pending 后继续调度；direct 模式不提供 Feedback rollback；Graph UI 不再常驻展示 Stop 能力边界说明，具体 Stop 操作结果或错误仍通过运行消息/状态反馈表达；操作后刷新面板并尽量保留 selected node；无可执行节点时返回结构化原因而不是静默成功。
