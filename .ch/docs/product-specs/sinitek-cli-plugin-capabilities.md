...
- Graph 当前不提供图编辑器、模板库、完整 human gate 表单、局部返工路径编辑或自动条件重规划；真实 CLI 进程停止仅在存在 active Graph run/node 映射时可同步触发
...
- Graph 运行系统消息会带 `openGraphRun` action，点击后打开独立 GraphRunPanel；Graph conversation tab 会显示 `🗺️` 标识，active Graph tab 的底部运行状态行固定提供“打开 Graph 图”按钮；主 Graph tab 的视觉运行态跟随整个图级生命周期，Graph 正式开始后保持运行中，直到 run 进入 `completed`、`error` 或 `stopped` 才释放，节点 tab 按各自执行流独立开始，并在节点 runner 结束后自动关闭。面板用 SVG 边/箭头和 HTML 节点按钮展示真正可视 DAG，并简化为上下两块：上方可视图约占主体 50% 高度，节点矩形保持紧凑且只显示中文标题和轻量状态；下方只展示当前选中节点详情、错误、attempt、prompt/artifact/通信文件和可用节点控制；不再渲染 run 概览、状态统计、节点列表、recent events 或 finalAnswer 区块。普通节点进入 `running` 后会立即刷新已打开面板，执行中节点使用主题变量驱动的流水边框动画。`openGraphRun` 可打开指定 run / node；不带 run id 时会从当前 workspace / CLI 的持久化 store 找最近 Graph run，坏 store 文件以非阻塞 diagnostics 展示。
- GraphRunPanel 只渲染真实接通且当前状态允许的操作：Continue / Resume sleeping、needs-review 或 error run，“我要说话”补充消息，Retry failed / blocked node，Feedback rollback failed / blocked 验证类节点，Approve human_gate，Stop run。补充消息会写入 Graph run 的 `supplementalRequirements`、主沟通文件和 events，并注入后续 Graph 节点 prompt；不承诺立即打断已经运行中的节点。Retry 会在该 run 的独立 worktree 中回滚到目标节点执行前 `baseCommit` 并清理未跟踪文件，再重新调度节点；Feedback rollback 会优先按 active `review_feedback` / `if_fail` 边或上游依赖选择可回退工作节点，回滚到该上游节点 `baseCommit`，并将上游节点及其下游重置为 pending 后继续调度；操作后刷新面板并尽量保留 selected node；无可执行节点时返回结构化原因而不是静默成功。
...
- Graph 当前限制：没有图编辑器、模板库、拖拽 DAG、运行前人工调整/确认、完整 human gate 表单、从失败节点自动生成补充需求、局部返工路径编辑或自动条件重规划；完成态合回不会自动提交、自动解决冲突或自动清理 worktree / Graph 分支。Stop 至少保证 Graph 状态和事件落盘为 stopped；只有 active CLI run 已携带 `graphRunId` / `graphNodeId` 映射时才会同时尝试停止真实 CLI 进程，缺少映射时会明确提示只是持久化 Graph 停止状态。
...
- 任务列表提取与展示；Claude 交互式运行除兼容 `TodoWrite` 外，也会根据 `TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet` / `TaskStop` 工具事件实时刷新任务列表；Codex app-server assistant 消息会从实际日志中的 `Tasklist:` / `Tasklist update:` / `Tasklist 更新：` 段落解析 `[completed]`、`[in_progress]`、`[pending]`、`[x]`、`[ ]` 与中文状态词，兼容多行列表和同行分号分隔。已解析的纯任务列表 assistant 气泡从对话中隐藏；普通说明混合 Tasklist 时，仅在展示文本中剥离任务列表片段并保留说明正文。OpenCode 会从 JSONL `tool_use` 的 `todowrite` 事件读取 `state.input.todos`，并兼容 metadata/output 结果，把 `content/status` 归一化为 `{ text, done }` 后实时刷新当前或并行对话 tab 的任务列表，显式空列表会清空本轮任务。任务列表标题提供收起/展开箭头；收起后仅保留标题、进度数量和箭头，进度显示为已完成/总数（例如 `2/4`），展开状态按 conversation tab 的运行时状态保留。OpenCode 同时通过专用 `taskListUpdate` 和对应 tool trace 元数据驱动浮层，运行中的会话消息刷新会保留 external 列表，Webview 状态重建后会重放仍在执行的列表
...
- Graph 运行系统消息中的“打开 Graph 运行图”action 会打开 GraphRunPanel；面板从 Graph run store 和 events 构建状态，但界面只保留上下两区：上方约 50% 高度的紧凑标题式可视 DAG，下方节点详情、补充消息和当前真实可用的 Continue、“我要说话”、Retry、Feedback rollback、Approve、Stop 控制。读取失败或 run 缺失时使用 i18n 错误/空态；Phase 2 起支持指定/最近 run 恢复打开。
