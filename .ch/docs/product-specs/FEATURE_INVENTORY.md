...
| AI 对话 / OpenCode | OpenCode CLI 支持 | active | 终端用户 | `.ch/docs/product-specs/sinitek-cli-plugin-capabilities.md`、`.ch/docs/references/cli-runtime-reference.md` | AI 对话 CLI 分组、命令/参数配置、配置中心、会话存档、模型/规则能力 | 待随源码子任务更新；至少需覆盖 `npm run build` 和相关 runner/config 单测 | OpenCode 已作为 Codex、Claude 之外的新支持目标；按插件通用 CLI 配置接入，尽量复用 Codex / Claude 已有配置中心、AI 对话、Loop、上下文压缩、模型与规则能力；具体 OpenCode 专属参数以当前实现为准 |
| AI 对话 / OpenCode | 精确模型动态推理 variant | active | 终端用户 | `.ch/docs/product-specs/sinitek-cli-plugin-capabilities.md`、`.ch/docs/references/cli-runtime-reference.md` | `src/cli/openCodeModelCapabilities.ts`、`src/modelSelectionStore.ts`、`src/panelStateBuilder.ts`、`src/extension.ts`、`src/cli/commandRunner.ts`、Webview thinking selector | `npm run build`；相关 OpenCode resolver/config/Webview/runner/action/integration tests | variants 只来自精确模型 CLI metadata 或当前配置显式声明；未知模型 Default-only，不按 npm/provider/model 名猜测；选择按 active config + 精确模型隔离，运行时仅为当前有效非空值传 `--variant`，显式 args 优先；`--thinking` 仅控制 thinking blocks |
...
| 配置 / 配置中心 | CLI 配置档案管理与卡片级保存 | active | 终端用户 | `.ch/docs/product-specs/sinitek-cli-plugin-capabilities.md` | 配置中心 webview、`media/config/assets/config-app-ui.js`、`src/config/configService.ts`、`src/webview/configPanel.ts` | `npm run build`；`node --check media/config/assets/config-app-ui.js` | 支持配置档案列表、排序、激活、删除、初始化、备份、导出、Skills 和 MCP 管理；配置编辑区取消顶部统一保存，改为 Claude settings、OpenCode 单 `config.json`、Codex config/auth 各卡片右上角独立保存；OpenCode 只维护 `~/.opencode/config.json`，不提供 `.env` 第二保存入口；Gemini 配置入口已移除 |
