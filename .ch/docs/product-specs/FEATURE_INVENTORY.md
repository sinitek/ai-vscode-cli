...
| 执行模式 | Codex / Claude 交互式续接 | active | 开发者 | `.ch/docs/product-specs/sinitek-cli-plugin-capabilities.md` | `src/interactive/*`, `src/extension.ts` | manual | 维护底层 thread/session 映射；Codex 遇到非主动中断/异常时会隐式发送“继续”自动重试，重试 5 次、每次间隔 30 秒，且不展示这条用户消息 |
