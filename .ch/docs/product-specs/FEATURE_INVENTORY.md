...
| 会话管理 | 多标签并行会话与历史记录 | active | 开发者 | `.ch/docs/product-specs/sinitek-cli-plugin-capabilities.md` | `src/extension.ts`, `src/webview/viewContent.ts` | script+manual | 含 session、tab、prompt history，默认仅保留最近 30 天，并覆盖旧工作区清理；切换单个 tab 的 CLI 分组/历史会话时不应中断其他 tab 的运行中任务；conversation tab 超过 5 个时，顶部标签区启用左右翻页按钮，每页最多显示 5 个 tab；队列仅在上一个任务成功结束后才继续，失败/停止会保留剩余提示词，并可在队列弹窗手动继续 |
