...
description: Use when you need a low-noise consolidation pass that suggests what should be compressed, extracted, or promoted from active plans, pitfalls, and memory hot-zone docs into the memory pyramid, pending items, active risks, lessons learned, runbooks, or design docs.
---

# Memory Consolidator

目标：把 active plan、pitfalls 和 memory 热区之间“哪些内容该压缩、抽取或上提”这件事，变成可重复执行、可审阅的半自动化流程。

## 什么时候用

- 一个非平凡任务准备收尾，需要决定哪些内容该进入长期记忆
- 一个任务完成后，需要检查 L1 滚动摘要、L2 事件记忆、L3 用户/项目画像、L4 程序性经验是否可以优化
- 最近刚新增或更新了 active plan、pitfalls、pending items、active risks、lessons
...
- 当前没有 active plan 或 pitfall 变化，热区记忆也没有待治理项

## 工作流

1. 在仓库根目录运行：
   - `python3 .agents/skills/memory-consolidator/scripts/consolidate_memory.py`
   - 如需完整候选清单，追加 `--max-suggestions-per-kind 0`；默认每类只保留前 20 条候选以降低 generated 报告噪音。
2. 打开生成结果：
...
4. 完成上提后，重新生成：
   - `memory-indexer`
   - 如果任务暴露 freshness 风险，手动复核对应事实来源的 front matter 和 open loops

## 产出要求

- 明确列出本次扫描了哪些 plan 和 pitfalls
...
- 不要让 active plan 长期替代 `memory/`、`runbooks/` 或 `design-docs/`
