...
龙虾模式已经很接近当前主流的 Agentic Coding Loop：用户给目标，AI 自己反复执行“计划 -> 编码 -> 执行 -> 验证 -> 修复”，用户更像是 human on the loop，负责设定目标、观察结果、在必要时介入。

当前工程已经具备不少循环能力：

- `coding / lobster` 顶层模式区分。
- 龙虾内部支持 `main_sub_multi_agent` 和 `debate_multi_agent`。
...
因此，建议把“目标 + 边界 + 验收”设计成龙虾任务创建时的结构化契约，并让 UI、任务记录、提示词、运行时校验、群聊展示和恢复逻辑都围绕这个契约工作。

## 2. 优化目标

边界设置优化不应该把龙虾模式变成繁琐审批系统。目标是让用户用较低成本给 AI 一个清晰的行动框：
...
建议在龙虾任务记录中新增一个结构化字段，概念上叫 `boundaryContract`。它不是替代原始 prompt，而是把原始 prompt 中最关键的行动边界提取成机器可读数据。
...
在龙虾模式输入区增加一个轻量的“边界卡片”，默认折叠，支持快速预设和高级编辑。
...
龙虾群聊面板应显示当前任务的边界摘要，而不是只显示对话时间线。建议放一个固定的“任务边界”区域：
...
- 如果命中 `writeDenylist` 或 `protectedPaths`，立即停止当前龙虾任务并进入 `needs-review`。
...
- 新建龙虾任务能在任务记录里看到 `boundaryContract`。
...
1. 龙虾任务记录新增 `boundaryContract`，包含目标、允许写入范围、禁止写入范围、验证命令、最大轮次。
