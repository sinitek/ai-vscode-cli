...
- Codex 交互模式当前会把 `sandbox/approval/model/add-dir/web search` 等核心参数映射到 `codex app-server --listen stdio://` 的 JSON-RPC 会话；握手阶段会发送 `initialize` + `initialized`，其中 `clientInfo` 统一声明为 `codex` 并优先探测本机已安装 Codex 版本，再通过 `thread/start` / `thread/resume` + `turn/start` 延续 threadId。
