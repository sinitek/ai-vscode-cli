...
这个目录是仓库内轻量 harness 知识系统。starter 保留规则、测试、ontology、product specs、memory、exec-plans 和 runbooks；其他主题等真实项目出现稳定事实后再增设。
...
- 运行与避坑导航并入 `.ch/docs/runbooks/README.md`。
...
- 新增目录前，优先判断是否能放进现有分类；确需新增时要说明为什么现有入口不足。
