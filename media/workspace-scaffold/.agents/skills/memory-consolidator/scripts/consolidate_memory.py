...
SNAPSHOT_SOURCE_FILES = (
    f"{MEMORY_DIR}/ROLLING_SUMMARY.md",
    f"{MEMORY_DIR}/EVENT_MEMORY.md",
...
def main() -> int:
    ...
    pitfall_entries = collect_pitfall_entries(root, privacy, timestamp_resolver)
    ...
    generated_at = source_snapshot_timestamp(root)
...
class SourceTimestampResolver:
    def __init__(self, root: Path) -> None:
        self.root = root
        self.cache: dict[str, str] = {}

    def iso_for(self, path: Path) -> str:
        key = path_for_git(self.root, path)
        if key not in self.cache:
            self.cache[key] = git_paths_timestamp(self.root, [key]) or iso_from_timestamp(path.stat().st_mtime)
...
def git_paths_timestamp(root: Path, paths: list[str]) -> str | None:
    try:
...
def collect_markdown_docs(
    ...
    timestamp_resolver: "SourceTimestampResolver",
    *,
    limit: int | None = None,
    ...
    if newest_first:
        files.sort(key=timestamp_resolver.iso_for, reverse=True)
    else:
        files.sort()
    ...
    for file_path in files:
        ...
        docs.append(
            MarkdownDoc(
                ...
                modified_at=timestamp_resolver.iso_for(file_path),
...
def collect_pitfall_entries(
    ...
    for file_path in sorted(pitfalls_root.glob("*.md")):
        ...
        if is_starter_text(body):
            continue
        entries.extend(parse_pitfall_entries(body, file_path, root, timestamp_resolver))
    return entries
    ...
    def flush_entry() -> None:
        ...
        entries.append(
            PitfallEntry(
                ...
                modified_at=timestamp_resolver.iso_for(path),
            )
        )
        current_title = None
        current_section = "_summary"
...
def source_snapshot_timestamp(root: Path) -> str:
    """Return a stable Git timestamp for the scanned consolidation source snapshot."""
    git_timestamp = git_paths_timestamp(root, [*SNAPSHOT_SOURCE_FILES, *SNAPSHOT_SOURCE_DIRS])
    if git_timestamp:
        return git_timestamp

    timestamps: list[datetime] = []
    for rel_path in SNAPSHOT_SOURCE_FILES:
        path = root / rel_path
        if path.is_file():
            timestamps.append(datetime.fromtimestamp(path.stat().st_mtime, tz=UTC))

    for rel_dir in SNAPSHOT_SOURCE_DIRS:
        path = root / rel_dir
        if not path.exists():
            continue
        for file_path in sorted(path.rglob("*.md")):
            if file_path.is_file() and file_path.name not in EXCLUDED_FILENAMES:
                timestamps.append(datetime.fromtimestamp(file_path.stat().st_mtime, tz=UTC))

    if not timestamps:
        return "1970-01-01T00:00:00Z"
    return max(timestamps).replace(microsecond=0).strftime("%Y-%m-%dT%H:%M:%SZ")
